...
- **Backend & DB**
    - Copy env files: `cp backend/.env.example backend/.env` and `cp db/.env.example db/.env` (if present).
    - Start stack: `make docker-up` (or `docker-compose up -d`).
    - The backend container runs `docker-start.sh`, which will install Composer deps, run `npm install`, run `php artisan migrate --force` and `php artisan db:seed --force`, and start the Reverb websocket server.

- **Frontend**
    - Copy env: `cp frontend/.env.example frontend/.env`.
    - In the frontend container (or locally with Node): `npm install` then `npm run dev`.
    - Vite dev server listens on port `5173` (see [docker-compose.yml](cci:7://file:///Users/moshe.shimanovich/Documents/parking-reservation/docker-compose.yml:0:0-0:0)).

- **Local dev without Docker (backend)**
    - From `backend/`: `composer install` then `cp .env.example .env` then `php artisan key:generate`.
    - Configure a PostgreSQL DB matching `.env`, then run `php artisan migrate --force`.
    - For the full dev stack (HTTP server, queue worker, logs, Vite), use `composer run dev`.
...
- **Conflict handling in code**:
    - [ReservationService::createReservation](cci:1://file:///Users/moshe.shimanovich/Documents/parking-reservation/backend/app/Services/ReservationService.php:65:4-117:5) wraps the insert in a transaction and catches `QueryException`.
    - Overlap violations are detected by [isOverlapConstraintViolation()](cci:1://file:///Users/moshe.shimanovich/Documents/parking-reservation/backend/app/Services/ReservationService.php:421:4-435:5) and mapped to `ReservationTimeConflictException`, which the controller returns as HTTP `409`.
- **Additional guards**:
...
- **WebSocket / broadcasting responsibilities**:
    ...
    - [ReservationService](cci:2://file:///Users/moshe.shimanovich/Documents/parking-reservation/backend/app/Services/ReservationService.php:26:0-464:1) computes only the affected slot cells for a reservation and broadcasts `ParkingSlotStatusChanged` events so clients can patch the UI without refetching the whole snapshot.
...
- **Division of labor**:
    - **REST**: initial data loads and commands (login, list spots, get daily availability, create/complete reservations).
    - **WebSockets**: incremental updates when reservations are created or completed, streaming only the impacted slot cells to all clients viewing that date.
...
- **Reservations & status model**
    ...
    - Business rules allow reservations only within the daily slot window derived from configured slot times (via [SlotService](cci:2://file:///Users/moshe.shimanovich/Documents/parking-reservation/backend/app/Services/SlotService.php:10:0-120:1)).
